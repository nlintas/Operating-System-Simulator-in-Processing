class SOS {
  ...
  SOS(Hardware h) {
    ...
    //Swap the schedulers around
    kernel.put("Scheduler", new RR("++2", "RR scheduler", this) );
    //Run the routines
  ...
  void step() {
    ...
    //Check if there is a process running and there is still time until the process does the RR interrupt
    //Or if there is no running process
    //Or if interrupts are enabled
    if (((runningProcess!=null && runningProcess.time_until_interrupt > 0) || runningProcess == null) || !interruptsEnabled) 
    {
      //Make sure to decrement the time until RR interrupt only if there is running process
      if (runningProcess != null)
      {
        runningProcess.time_until_interrupt --;
      }
      currentAddress = physicalAddress;
    ...
    } else
    {
      ...
      queue.add(queue.size(), runningProcess);
